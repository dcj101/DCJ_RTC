...
// io watch 是 ev 和 xrtc server的桥梁
...
void EventLoop::start_io_event(IOWatcher* w, int fd, int mask) {
    ...
    if (ev_is_active(io)) {
        ...
        int new_events = active_events | mask; // 【修改1】重命名变量，避免重复定义
        if (new_events == active_events) {
            return;
        }
        int libev_events = TRANS_TO_EV_MASK(new_events); 
        ev_io_stop(_loop, io);
        ev_io_set(io, fd, libev_events);
        ev_io_start(_loop, io);
    } else {
        int libev_events = TRANS_TO_EV_MASK(mask);
        ev_io_set(io, fd, libev_events);
        ev_io_start(_loop, io);
    }
}
...
// timer是独立的eventloop
TimerWatcher* 
EventLoop::create_timer(time_cb_t cb, void* data, bool need_repeat) {
