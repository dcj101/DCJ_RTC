...
int RtcWorker::send_rtc_msg(std::shared_ptr<RtcMsg> msg) {
     // 将消息投递到worker的队列
    RTC_LOG(LS_INFO) << "rtc worker send rtc msg, worker_id: " << _worker_id 
        << ", cmdno: " << msg->cmdno << ", log_id: " << msg->log_id;
    push_msg(msg);
    return notify(RTC_MSG);
}

void RtcWorker::_process_notify(int msg) {
    RTC_LOG(LS_INFO) << "rtc worker receive notify, worker_id: " << _worker_id << ", msg: " << msg;
     switch (msg) {
        case QUIT:
            _stop();
            break;
